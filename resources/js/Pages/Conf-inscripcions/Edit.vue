<template>
    <Head title="Editar Configuración de Inscripción" />

    <AuthenticatedLayout>
        <template #header>
            Editar Configuración de Inscripción
        </template>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 border-b border-gray-200">
                <form @submit.prevent="$event => form.patch(route('conf-inscripcions.update', confinscripcion))" >
            ...
            <div  class="overflow-hidden mt-2 sm:rounded-lg flex flex-row space-x-2 mb-2 justify-end">
              <DangerButton @click="$event => deleteConf(confinscripcion.id)">
                    Eliminar
              </DangerButton>
            </div>
        </div>
...
const props = defineProps({
    errors: { type: Object },
    confinscripcion: { type: Object },
    periodos: {type:Object},
});

const form = useForm({
    periodo_id: props.confinscripcion.periodo_id,
    fecha_ini: props.confinscripcion.fecha_ini,
    fecha_fin: props.confinscripcion.fecha_fin,  
    
});

const deleteConf = (id) => {
  if (confirm('Está seguro de eliminar esta configuración?')) {
    form.delete(route('conf-inscripcions.destroy', id));
  }
}
onMounted(() => {
